...
def get_tweet(input_text):
    ...
    response = groq_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": "Summarize the following context and write a humorous 130 character tweet predicting the outcome of the following match.  Include only relevant stats and odds for the game in question do not make up any details. Use approprate hashtags and emojis." + " " + input_text},
        ],
        temperature=0.3, 
        max_tokens=200
    )
